          ...
          if (uPart.match(urlRegex)) {
              ...
              return (
                 <React.Fragment key={`${i}-${j}-${k}`}>
                   <a href={href} target="_blank" rel="noreferrer" className="text-indigo-400 hover:text-indigo-300 break-all hover:underline decoration-indigo-400/30">
                     {url}
                   </a>
                   {suffix}
                 </React.Fragment>
        ...
        if (numMatch) {
           return (
            <div key={index} className="flex gap-3 mb-3 ml-1">
               <span className="flex-shrink-0 bg-indigo-500/10 text-indigo-400 font-bold text-xs h-6 w-6 rounded-full flex items-center justify-center mt-0.5 border border-indigo-500/20">
                 {numMatch[1]}
               </span>
               <div className="text-slate-300 leading-relaxed flex-1 pt-0.5">
...
export const EventFeed: React.FC = () => {
  ...
  const handleCardClick = async (event: TechEvent) => {
    ...
    // Reset details but keep loading true
    setDetails('');
    ...
    <div className="max-w-7xl mx-auto px-4 py-12 relative">
      ...
      {selectedEvent && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={closeModal}>
          ...
          <div 
            className="bg-slate-900 w-full max-w-2xl max-h-[85vh] overflow-hidden rounded-[2rem] shadow-2xl flex flex-col relative z-10 transform transition-all animate-in fade-in zoom-in-95 duration-200 border border-white/10" 
                  ...
                  {/* Basic Info While Loading */}
                   ...
                   <div className="space-y-4 animate-pulse">
                      ...
                      <div className="flex items-center justify-center mt-8">
                         <span className="text-xs text-indigo-400 font-semibold tracking-wider uppercase animate-pulse">AI is compiling details...</span>
                      </div>
                   </div>
                </div>
              ) : (
               ...
               <div className="flex gap-3">
                 {selectedEvent.sourceUrl && (
                   <a 
                     ...
                     target="_blank" 
                     rel="noreferrer"
                     className="px-5 py-2.5 bg-white hover:bg-slate-200 text-slate-900 font-bold rounded-xl shadow-lg shadow-white/10 transition-all active:scale-95 flex items-center gap-2"
                   >
                     Official Page
